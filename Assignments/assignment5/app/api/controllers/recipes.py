...
from ..models.schemas import RecipeBase

def create(db: Session, recipe):
    db_recipe = models.Recipe(
        recipe_id = recipe.id,
        recipe_resource_id = recipe.recipe_resource_id,
    )
    db.add(db_recipe)
    db.commit()
...
def update(db: Session, recipe_id, recipie):
    db_recipe = db.query(models.Recipe).filter(models.Recipe.id == recipe_id)
    update_data = recipie.model_dump(exclude_unset=True)
